	...
	Janus.init({debug: "all", callback: function() {
		...
		$('#start').one('click', function() {
			...
			if(!Janus.isWebrtcSupported()) {
				bootbox.alert("Geen WebRTC support... ");
				return;
			}
			// Create session
			janus = new Janus(
						...
						janus.attach(
							{
								...
								webrtcState: function(on) {
									...
									if(on) {
										bootbox.alert("<b> Je schermdeelsessie is net gestart: deel de sessie identifier <b>" + room + "</b> met andere deelnemers");
									} else {
										bootbox.alert("Je schermdeelsessie is net gestopt.", function() {
											janus.destroy();
								...
								onmessage: function(msg, jsep) {
									...
									if(event != undefined && event != null) {
										if(event === "joined") {
											...
											Janus.log("Succesvol verbonden op " + msg["room"] + " met ID " + myid);
											if(role === "publisher") {
...
function preShareScreen() {
	...
	if(!Janus.isExtensionEnabled()) {
		bootbox.alert("Je gebruikt een recente versie van Chrome, maar je hebt de schermdelen extensie niet geïnstalleerd. Klik <b><a href='https://chrome.google.com/webstore/detail/janus-webrtc-screensharin/hapfgfdkleiggjjpfpenajgdnfckjpaj' target='_blank'>hier</a></b> om deze te installeren.", function() {
			window.location.reload();
		});
		return;
	}
	...
	if($('#desc').val() === "") {
		bootbox.alert("Geef een omschrijving van de room");
	...
	if(navigator.mozGetUserMedia) {
		...
		bootbox.dialog({
			title: "Volledig scherm delen of enkel een venster?",
			message: "Firefox behandelt schermdelen op een andere manier: wil je gans je scherm delen, of wens je liever één venster of applicatie te delen?",
			buttons: {
				screen: {
					label: "Share screen",
...
//Welconsult functions
//function bindFullscreen(video) {
//	$("screenvideo").unbind('click').click(toggleFullScreen);
//}
